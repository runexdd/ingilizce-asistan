...
/**
 * `[kelime, türkçe, örnek]` üçlüleri. Örnekler bilerek kısa: `LEVEL_SPEC`
 * A1 için 6, A2 için 8 kelimelik örnek sınırı koyuyor.
 */
...
const A1: Row[] = [
  ...
  ['open', 'açık, açmak', 'The shop is open now.'],
...
const A2: Row[] = [
  ...
  ['tired', 'yorgun', 'I was too tired to cook.'],
  ...
  ['clean', 'temiz, temizlemek', 'The room was clean and bright.'],
  ...
  ['friendly', 'cana yakın, dost canlısı', 'Our new colleague is friendly.'],
  ...
  ['change', 'değiştirmek, değişim', 'I changed my plans.'],
  ...
  ['agree', 'katılmak, aynı fikirde olmak', 'I agree with you.'],
...
const B1: Row[] = [
  ...
  ['knowledge', 'bilgi, birikim', 'Her knowledge of the market is impressive.'],
  ...
  ['encourage', 'teşvik etmek, cesaretlendirmek', 'My teacher encouraged me to try again.'],
...
const B2: Row[] = [
  ...
  ['attempt', 'girişim, denemek', 'His first attempt was unsuccessful.'],
  ...
  ['make sense', 'mantıklı olmak, anlam ifade etmek', 'That explanation makes sense now.'],
...
function build(rows: Row[], level: CEFRLevel): BankWord[] {
  return rows.map(([word, meaning, example]) => ({ word, level, meaning, example }));
}
...
for (const entry of WORD_BANK) {
  BY_KEY.set(key(entry.word), entry);
}
 ...
 * bıraktığı günü bambaşka kelimelerle bulur.
 */
...
export function wordsForLevel(
  ...
  return WORD_BANK.filter((w) => !skip.has(key(w.word)) && rank(w) < 99)
